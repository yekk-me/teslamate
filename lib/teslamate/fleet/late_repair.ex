defmodule TeslaMate.Fleet.LateRepair do
  ...
  def attempt(event, checkpoint, base) do
    with :ok <- eligible(event),
         ...
         vehicle <- %{
           vehicle
           | drive_state: %{vehicle.drive_state | timestamp: event.recorded_at}
         },
      ...
      for key <- ~w(id start_date end_date start_position_id end_position_id)a do
        if Map.fetch!(drive, key) != Map.fetch!(updated, key),
          do: Repo.rollback(:repair_changed_boundary)
      end
      ...
      Repo.insert!(%Repair{
        ...
        before_metrics: metrics(drive),
        after_metrics: metrics(updated),
  ...
  def retry(base, limit \\ 10) do
    ids =
      Repo.all(
        from e in Event,
          where:
            e.car_id == ^base.car.id and e.status == "late" and
    ...
    Enum.map(ids, fn id ->
      Repo.transaction(fn ->
        ...
        event =
          Repo.one!(
            from e in Event, where: e.id == ^id and e.car_id == ^base.car.id, lock: "FOR UPDATE"
          )
  ...
  defp eligible(%Event{source: "telemetry", payload: %{"data" => data}}) do
    ...
    cond do
      not Enum.all?(keys, &(&1 in @fields)) ->
        {:error, :unsupported_fields_or_transition}

      not Enum.any?(keys, &(&1 in ~w(Location Odometer VehicleSpeed))) ->
        {:error, :not_a_driving_sample}
  ...
  defp context(event, checkpoint) do
    anchor =
      Repo.one(
        from e in Event,
          where:
            e.car_id == ^event.car_id and e.source == "snapshot" and e.status in ^@processed and
              e.recorded_at < ^event.recorded_at,
          order_by: [desc: e.recorded_at, desc: e.id],
          limit: 1
      )
    ...
    if anchor && DateTime.diff(event.recorded_at, anchor.recorded_at, :second) <= 86_400 do
      events =
        Repo.all(
          from e in Event,
            where:
              e.car_id == ^event.car_id and e.recorded_at >= ^anchor.recorded_at and
                e.recorded_at <= ^checkpoint.recorded_at,
            order_by: [asc: e.recorded_at, asc: e.id],
            limit: 5001
        )

      {past, future} =
        Enum.split_while(events, &(DateTime.compare(&1.recorded_at, event.recorded_at) == :lt))
  ...
  defp merge(%Event{source: "telemetry"} = e, snapshot),
    do: Decoder.merge(snapshot, e.payload, e.recorded_at)

  defp merge(%Event{source: "status", payload: payload}, snapshot),
    do: Map.put(snapshot, "state", payload["state"])
  ...
  defp safe_delta(event, prior, next) do
    ...
    overwritten? =
      case next.source do
        "snapshot" ->
          Decoder.ready?(next.payload) &&
            Enum.all?(
              ~w(drive_state charge_state climate_state vehicle_state vehicle_config),
              &is_map(next.payload[&1])
            )

        "telemetry" ->
          MapSet.subset?(keys, MapSet.new(Enum.map(next.payload["data"], & &1["key"])))
    ...
    cond do
      prior["state"] != "online" || gear not in ~w(D N R) ->
        {:error, :unknown_driving_state}

      get_in(updated, ["drive_state", "shift_state"]) != gear ->
        {:error, :state_transition_requires_replay}

      not overwritten? ->
        {:error, :would_change_following_samples}
  ...
  defp closed_drive(event) do
    drives =
      Repo.all(
        from d in Log.Drive,
          where:
            d.car_id == ^event.car_id and d.start_date < ^event.recorded_at and
              d.end_date > ^event.recorded_at,
          lock: "FOR UPDATE",
          limit: 2
      )
    ...
    case drives do
      ...
      _ ->
        {:error, :overlapping_drives}
    end
  end

  defp unused_timestamp(event) do
    if Repo.exists?(
         from p in Log.Position, where: p.car_id == ^event.car_id and p.date == ^event.recorded_at
  ...
  defp odometer_between_neighbors(drive, attrs) do
    before =
      Repo.one(
        from p in Log.Position,
          where: p.drive_id == ^drive.id and p.date < ^attrs.date,
    ...
    if before && after_position && is_number(before.odometer) &&
         is_number(after_position.odometer) && is_number(attrs.odometer) &&
         before.odometer <= attrs.odometer && attrs.odometer <= after_position.odometer,
       do: :ok,
       else: {:error, :inconsistent_odometer}
  end

  defp retain(event, reason) do
    Repo.update!(
      Ecto.Changeset.change(event, status: "late", error: "repair_" <> Atom.to_string(reason))
    )
