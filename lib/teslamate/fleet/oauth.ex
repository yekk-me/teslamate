defmodule TeslaMate.Fleet.OAuth do
  ...
  def begin(tenant_id) do
    ...
    with {:ok, url} <- TeslaApi.Fleet.authorization_url(state) do
      TenantContext.run(tenant_id, fn ->
        ...
        Repo.delete_all(from(s in "fleet_oauth_states", where: s.expires_at < ^now), prefix: "private")
        Repo.insert_all("fleet_oauth_states", [%{digest: digest(state),
          expires_at: DateTime.add(now, 600, :second)}], prefix: "private")
      end)
  ...
  def consume(tenant_id, state) when is_binary(state) and byte_size(state) in 32..128 do
    TenantContext.run(tenant_id, fn ->
      hash = digest(state)
      now = DateTime.utc_now()
      case Repo.delete_all(from(s in "fleet_oauth_states",
             where: s.digest == ^hash and s.expires_at > ^now), prefix: "private") do
        {1, _} -> :ok
