defmodule TeslaMate.MultiTenant.SharedDatabase do
  ...
  def enabled?, do: TeslaMate.MultiTenant.enabled?() and System.get_env("TESLAMATE_SHARED_DATABASE") == "true"
  def valid_schema?(schema), do: is_binary(schema) and Regex.match?(~r/^tenant_[a-z0-9_]{1,48}$/, schema)
  ...
  def scope(fun) do
    if enabled?() and not scoped?() do
      ...
      if enabled?() and Process.get(@scope_key) != schema!(), do: raise("cannot switch tenants inside a transaction")
      fun.()
    end
  end
  ...
  def enter(fun) do
    ...
    try do
      ...
      Ecto.Adapters.SQL.query!(TeslaMate.Repo, "SET LOCAL search_path TO \"#{schema}\", public", [])
      fun.()
    after
  ...
  def migrate(schema) do
    ...
    if TeslaMate.MultiTenant.enabled?(), do: raise("migrations require an isolated maintenance process")
    ...
    {:ok, repo} = TeslaMate.Repo.start_link(Keyword.merge(opts, pool_size: 2, after_connect: after_connect))
    try do
