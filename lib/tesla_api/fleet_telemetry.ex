defmodule TeslaApi.FleetTelemetry do
    ...
    case System.get_env("TESLA_FLEET_COMMAND_PROXY") do
      url when is_binary(url) ->
        case URI.parse(url) do
          ...
          when is_binary(host) and path in [nil, "", "/"] -> {:ok, String.trim_trailing(url, "/")}
          _ -> {:error, %Error{reason: :invalid_command_proxy}}
        end
      _ -> {:error, %Error{reason: :command_proxy_not_configured}}
    end
  end
  ...
  def run(%Auth{} = auth, vin, :configure) do
    with {:ok, proxy} <- proxy_url(),
         ...
         {:ok, result} <- request(auth, :post, proxy <> "/api/1/vehicles/fleet_telemetry_config", %{"vins" => [vin], "config" => config}) do
      skipped = Map.get(result, "skipped_vehicles", %{})
      if Enum.any?(skipped, fn {_reason, vehicles} -> vehicles not in [[], nil] end),
  ...
  def run(%Auth{} = auth, vin, action) when action in [:status, :errors] do
    ...
    request(auth, :get, Fleet.api_url() <> "/api/1/vehicles/" <> URI.encode(vin, &URI.char_unreserved?/1) <> "/" <> suffix, nil)
  end

  defp vehicle_config do
    with path when is_binary(path) <- System.get_env("TESLA_FLEET_VEHICLE_CONFIG_FILE"),
         {:ok, body} <- File.read(path),
         {:ok, %{"hostname" => host, "ca" => ca, "fields" => fields} = config} <- Jason.decode(body),
  ...
  defp request(auth, method, url, body) do
    ...
    case Tesla.request(client, method: method, url: url, body: body) do
      {:ok, %Tesla.Env{status: 200, body: %{"response" => result}}} when is_map(result) -> {:ok, result}
      {:ok, %Tesla.Env{status: 401}} -> {:error, %Error{reason: :unauthorized}}
      {:ok, %Tesla.Env{status: status}} -> {:error, %Error{reason: :telemetry_request_failed, message: "Fleet telemetry HTTP #{status}"}}
      {:error, _} -> {:error, %Error{reason: :telemetry_request_failed}}
