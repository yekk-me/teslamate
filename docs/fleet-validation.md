...
## 迟到补录与共享服务后续验证（2026-09-23）

共享分支首轮提交 `dd9bb2bdb001ea7fd7f58bdb03395786652618bb` 的 [CI 35871856466](https://github.com/yekk-me/teslamate/actions/runs/35871856466) 通过全部三个专用任务：393 项 ExUnit（27 项 Fleet/OAuth）、9 项 Python、两租户独立补录、历史复制、官方接收器/Kafka/HTTP/数据库联调、实际 release 镜像及受限租户启动。

新增回归验证顺序到达与迟到补录的派生指标/位置一致、checkpoint 不回退、行程边界与 ID 不变、开放行程关闭后重试、重复投递、换挡/后续状态影响/里程异常/同时间冲突拒绝，以及审计写失败的完整回滚。自动修复范围见部署指南；这里的通过不能覆盖通用历史重建，也不能替代真实中国车辆验收。

配套 API 提交 `92685e15093c86382026ba63fefaf89a2ea65e69` 的 [CI 35872752633](https://github.com/yekk-me/teslamateapi/actions/runs/35872752633) 通过真实 PostgreSQL/MQTT 下的完整 Go race 回归及非 root 镜像验证。新增实际 HTTP 行程列表/详情/最新行程用例覆盖功率 NULL、真实零值、完整负功率积分、部分缺失与跨分钟间隔；审计刷新用例覆盖故障不推进进度、重试、幂等、用户分类与通勤 ID 保留。
