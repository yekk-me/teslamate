...
WORK_DIR="${SMOKE_WORK_DIR:-.local/multi-tenant-control-smoke}"
NETWORK="${SMOKE_DOCKER_NETWORK:-tm-mt-control-smoke}"
POSTGRES_CONTAINER="${SMOKE_POSTGRES_CONTAINER:-tm-mt-control-postgres}"
PGBOUNCER_CONTAINER="${SMOKE_PGBOUNCER_CONTAINER:-tm-mt-control-pgbouncer}"
MQTT_CONTAINER="${SMOKE_MQTT_CONTAINER:-tm-mt-control-mqtt}"
CONTROL_CONTAINER="${SMOKE_CONTROL_CONTAINER:-tm-mt-control-plane}"
APP_CONTAINER="${SMOKE_APP_CONTAINER:-tm-mt-control-app}"
...
TENANT_ID="${SMOKE_TENANT_ID:-tenant-real-a}"
TENANT_DB="${SMOKE_TENANT_DATABASE:-teslamate_mt_real_a}"
TENANT_DIRECTORY="$WORK_DIR/tenant-directory.json"
AUTHORIZE_RESPONSE="$WORK_DIR/authorize-response.json"
HEX_ARCHIVE="${HEX_ARCHIVE:-/tmp/hex-2.4.2-otp-26.ez}"
DIRECTORY_TOKEN="${TESLAMATE_TENANT_DIRECTORY_TOKEN:-control-directory-token}"
INTERNAL_TOKEN="${TESLAMATE_INTERNAL_API_TOKEN:-teslamate-internal-token}"
...
cleanup() {
  docker rm -f "$APP_CONTAINER" "$CONTROL_CONTAINER" "$POSTGRES_CONTAINER" \
    "$PGBOUNCER_CONTAINER" "$MQTT_CONTAINER" >/dev/null 2>&1 || true
...
mix_docker() {
  if [[ -n "${MIX_DOCKER_NAME:-}" ]]; then
    docker run --rm \
      ...
      -e HTTP_BINDING_ADDRESS=0.0.0.0 \
      -e PORT=4000 \
      -e HEX_OFFLINE=1 \
      ...
      -e TESLAMATE_INTERNAL_API_TOKEN="$INTERNAL_TOKEN" \
      -e TESLAMATE_INTERNAL_AUTHORIZE_URL="http://$APP_CONTAINER:4000/api/internal/tenants/$TENANT_ID/authorize" \
      -e TENANT_DIRECTORY_PATH="/work/$TENANT_DIRECTORY" \
      -e TENANT_AUTHORIZE_RESPONSE_PATH="/work/$AUTHORIZE_RESPONSE" \
...
control_docker() {
  docker run --rm \
    ...
    -e LOGGER_LEVEL="${LOGGER_LEVEL:-warning}" \
    -e HEX_OFFLINE=1 \
    -e CONTROL_TOKEN="$DIRECTORY_TOKEN" \
    -e DIRECTORY_PATH="/work/$TENANT_DIRECTORY" \
    "$ELIXIR_IMAGE" sh -lc "mix archive.install $HEX_ARCHIVE --force >/dev/null && (mix local.rebar --force >/dev/null 2>&1 || true) && mix run --no-start scripts/multi_tenant_control_server.exs"
}
...
MIX_DOCKER_NAME="$APP_CONTAINER" mix_docker "TESLAMATE_MULTI_TENANT=true \
  TESLAMATE_NODE_ID=control-smoke-node \
  TESLAMATE_TENANT_DIRECTORY_URL=http://$CONTROL_CONTAINER:8080/tenants \
  TESLAMATE_TENANT_DIRECTORY_TOKEN=$DIRECTORY_TOKEN \
  TESLAMATE_TENANT_SYNC_INTERVAL_MS=5000 \
...
mix_docker "mix run --no-start scripts/multi_tenant_control_authorize.exs"
...
echo "Control-plane smoke test finished. Runtime files are under $WORK_DIR"
