defmodule TeslaApi.FleetTelemetryTest do
  ...
  setup do
    ...
    File.write!(path, Jason.encode!(%{hostname: "telemetry.example", ca: "-----BEGIN CERTIFICATE-----", fields: %{Location: %{interval_seconds: 1}}}))
    values = %{"TESLA_FLEET_COMMAND_PROXY" => "https://fleet-command:4443", "TESLA_FLEET_VEHICLE_CONFIG_FILE" => path}
  ...
  test "configuration is server-owned and signed via the trusted proxy" do
    with_mock Tesla, [:passthrough], request: fn _, args ->
      ...
      {:ok, %Tesla.Env{status: 200, body: %{"response" => %{"updated_vehicles" => 1, "skipped_vehicles" => %{}}}}}
    end do
      assert {:ok, %{"updated_vehicles" => 1}} = FleetTelemetry.run(%Auth{token: "secret"}, "LRW00000000000001", :configure)
    end
  end

  test "skipped vehicles and unsafe proxy URLs are failures" do
    with_mock Tesla, [:passthrough], request: fn _, _ ->
      {:ok, %Tesla.Env{status: 200, body: %{"response" => %{"skipped_vehicles" => %{"missing_key" => ["VIN"]}}}}}
    end do
      assert {:error, %Error{reason: :vehicle_configuration_skipped}} = FleetTelemetry.run(%Auth{token: "secret"}, "VIN", :configure)
    end
    for url <- ["http://fleet-command", "https://user:password@fleet-command", "https://fleet-command/path", "https://fleet-command?target=other"] do
  ...
  test "status uses China API and never leaks failing response or token" do
    with_mock Tesla, [:passthrough], request: fn _, args ->
      assert args[:url] == TeslaApi.Fleet.api_url() <> "/api/1/vehicles/VIN/fleet_telemetry_config"
