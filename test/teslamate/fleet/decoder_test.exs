defmodule TeslaMate.Fleet.DecoderTest do
  ...
  test "delta fields preserve miles, mph, coordinates, precision and omitted values" do
    decoded = Decoder.merge(snapshot(@car), record(@car, 1, %{"VehicleSpeed" => 12.3456, "Odometer" => 10001.123456,
      "Location" => %{"latitude" => 31.123456789, "longitude" => 121.123456789}, "Gear" => "ShiftStateD"}), @at)
  ...
  test "invalid differs from omitted and unknown gear cannot end a drive" do
    ...
    unknown = Decoder.merge(snapshot(@car), record(@car, 1, %{"Gear" => "ShiftStateUnknown"}), @at)
    refute Decoder.ready?(unknown)
  end

  test "AC charger-side energy is never substituted for battery-side energy" do
    decoded = Decoder.merge(snapshot(@car), record(@car, 1, %{"DetailedChargeState" => "DetailedChargeStateCharging",
      "ACChargingEnergyIn" => 12.345, "DCChargingEnergyIn" => 10.123456, "ACChargingPower" => 7.4}), @at)
  ...
  test "new charge session cannot reuse previous session energy" do
    decoded = Decoder.merge(snapshot(@car), record(@car, 1, %{"DetailedChargeState" => "DetailedChargeStateCharging"}), @at)
