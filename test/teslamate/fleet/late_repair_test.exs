defmodule TeslaMate.Fleet.LateRepairTest do
  ...
  setup do
    ...
    base = %TeslaMate.Vehicles.Vehicle.Data{
      car: car, fleet?: true, import?: true,
  ...
  test "late interior samples match ordered projection without moving live state or session IDs", %{base: base, car: car} do
    before = finish(base)
    ...
    assert Map.take(before, [:id, :start_date, :end_date, :start_position_id, :end_position_id]) ==
      Map.take(after_drive, [:id, :start_date, :end_date, :start_position_id, :end_position_id])
    ...
    keys = [:start_date, :end_date, :distance, :duration_min, :speed_max, :power_max,
      :power_min, :outside_temp_avg, :inside_temp_avg, :ascent, :descent,
    ...
    positions = fn id ->
      ...
      |> Enum.map(&Map.take(&1, [:date, :odometer, :latitude, :longitude, :speed, :power, :battery_level]))
    end
  ...
  test "open-drive sample waits for closure then repairs once", %{base: base, car: car} do
    ...
    assert Repo.one!(from e in Event, where: e.status == "late").error == "repair_waiting_for_closed_drive"
  ...
  test "a late shift boundary cannot silently split an existing drive", %{base: base} do
    ...
    assert Repo.one!(from e in Event, where: e.status == "late").error == "repair_state_transition_requires_replay"
  ...
  test "a changed field carried into following samples requires full replay", %{base: base} do
    ...
    assert Repo.one!(from e in Event, where: e.status == "late").error == "repair_would_change_following_samples"
    assert Repo.aggregate(Repair, :count) == 0
  end
  ...
  test "odometer outliers and timestamp collisions remain auditable raw events", %{base: base} do
    ...
    assert Repo.one!(from e in Event, where: e.status == "late").error == "repair_inconsistent_odometer"
    assert {:ok, :late} = push(base, 61, %{"Odometer" => 10001.5})
    assert Repo.exists?(from e in Event, where: e.error == "repair_ambiguous_timestamp")
    assert Repo.aggregate(Repair, :count) == 0
  end

  test "an audit failure rolls back the sample and recalculated drive together", %{base: base, car: car} do
    before = finish(base)
    count = Repo.aggregate(Log.Position, :count)
    Repo.query!("CREATE FUNCTION reject_fleet_repair() RETURNS trigger LANGUAGE plpgsql AS $$ BEGIN RAISE EXCEPTION 'repair audit unavailable'; END $$")
    Repo.query!("CREATE TRIGGER reject_fleet_repair BEFORE INSERT ON fleet_repairs FOR EACH ROW EXECUTE FUNCTION reject_fleet_repair()")
    {:ok, :stored} = Ingest.store(car, "telemetry", record(car, 61, %{"Odometer" => 10001.123456, "VehicleSpeed" => 60.0}))
