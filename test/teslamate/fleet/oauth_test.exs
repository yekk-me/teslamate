defmodule TeslaMate.Fleet.OAuthTest do
  ...
  setup do
    config = %{"TESLA_FLEET_CLIENT_ID" => "client", "TESLA_FLEET_CLIENT_SECRET" => "secret",
    ...
    on_exit(fn -> Enum.each(original, fn {key, value} ->
  ...
  test "state is random, one-use and expires" do
    ...
    refute state == other
    Repo.update_all(from(s in "fleet_oauth_states"), [set: [expires_at: ~U[2000-01-01 00:00:00.000000Z]]], prefix: "private")
