defmodule TeslaMate.MultiTenant.SharedDatabaseTest do
  ...
  setup do
    ...
    unless Process.whereis(TeslaMate.MultiTenant.Registry), do: start_supervised!({Registry, keys: :unique, name: TeslaMate.MultiTenant.Registry})
    for name <- ["tenant_isolation_a", "tenant_isolation_b"] do
      ...
      Ecto.Adapters.SQL.query!(Repo, "CREATE TABLE #{name}.isolation (id integer PRIMARY KEY, value text)", [])
      Ecto.Adapters.SQL.query!(Repo, "ALTER TABLE #{name}.isolation OWNER TO #{name}", [])
      {:ok, tenant} = Tenant.new(%{id: name, database: %{host: "localhost", username: "test", password: "test", name: "test", schema: name}})
    ...
    on_exit(fn ->
      for {key, value} <- [{"TESLAMATE_MULTI_TENANT", old_multi}, {"TESLAMATE_SHARED_DATABASE", old_shared}] do
  ...
  test "same IDs, raw SQL and rollback remain within the tenant role" do
    ...
    for name <- ["tenant_isolation_a", "tenant_isolation_b"] do
      TenantContext.run(name, fn ->
        ...
        assert {:error, :deliberate} = Repo.transaction(fn ->
  ...
  test "cannot enter another tenant while retaining a leased transaction" do
    TenantContext.run("tenant_isolation_a", fn ->
      assert_raise RuntimeError, ~r/cannot switch tenants/, fn ->
        Repo.transaction(fn -> TenantContext.run("tenant_isolation_b", fn -> Repo.query!("SELECT 1") end) end)
      end
    end)
  end
end
