    ...
    test "interprets a significant offline period while driving with SOC gains as charge session",
      ...
      events =
        [
          ...
          [
            drive_event(now_ts + (:timer.minutes(5) + 1), 0.2, 20, 80, 300, 45),
            {:ok,
             online_event(
               drive_state: %{
                 timestamp: now_ts + (:timer.minutes(5) + 1) + 1,
                 latitude: 0.3,
                 longitude: 0.3
               }
             )}
      ...
      end_date =
        now
        |> DateTime.add(5 * 60 * 1000 + 1, :millisecond)
        |> DateTime.truncate(:millisecond)
      ...
      d1 = DateTime.from_unix!(now_ts + (:timer.minutes(5) + 1), :millisecond)
      ...
      d2 = DateTime.from_unix!(now_ts + (:timer.minutes(5) + 1) + 1, :millisecond)
    ...
    test "logs a drive after a significant offline period while driving",
      ...
      events =
        [
          ...
          List.duplicate({:ok, %TeslaApi.Vehicle{state: "offline"}}, 20) ++
          [
            drive_event(now_ts + (:timer.minutes(15) + 1), 0.2, 20, 19, 190, nil),
            {:ok,
             online_event(
               drive_state: %{
                 timestamp: now_ts + (:timer.minutes(15) + 1) + 1,
                 latitude: 0.3,
                 longitude: 0.3
               }
             )}
      ...
      d1 = DateTime.from_unix!(now_ts + (:timer.minutes(15) + 1), :millisecond)
      assert_receive {:start_state, ^car, :online, date: ^d1}
      assert_receive {:insert_position, ^car, %{}}
      assert_receive {:pubsub, {:broadcast, _, _, %Summary{state: :online}}}
      ...
      d2 = DateTime.from_unix!(now_ts + (:timer.minutes(15) + 1) + 1, :millisecond)
      assert_receive {:start_state, ^car, :online, date: ^d2}
      assert_receive {:insert_position, ^car, %{}}
      assert_receive {:pubsub, {:broadcast, _, _, %Summary{state: :online}}}
