...
{:ok, _} = Repo.start_link(Keyword.merge(opts, name: TenantSupervisor.repo_name("smoke"), pool: DBConnection.ConnectionPool))
tenant = %Tenant{id: "smoke", database: nil, vehicles: [%Tenant.Vehicle{id: "9001", vin: "LRW00000000000001", status: "active"}]}
{:ok, _} = TenantState.start_link(tenant: tenant)
base = TenantContext.run("smoke", fn ->
...
Stream.repeatedly(fn ->
  TenantContext.run("smoke", fn ->
    ...
    if File.exists?(Path.join(path, "check")) do
      import Ecto.Query
      counts = Repo.all(from e in Event, group_by: e.status, select: {e.status, count(e.id)}) |> Map.new()
      if (counts["projected"] || 0) >= 4 do
        ...
        true = positions >= 3
        File.write!(Path.join(path, "result.tmp"), Jason.encode!(%{events: 4, drives: 1, distance_km: drive.distance, positions: positions, statuses: counts}))
