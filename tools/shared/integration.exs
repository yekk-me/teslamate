...
copy_schema =
  "tenant_" <>
    (Base.encode16(:crypto.hash(:sha256, "full-copy-check"), case: :lower) |> String.slice(0, 32))

for {schema, vin} <- [
      {"tenant_migration_check", "LRW00000000000001"},
      {copy_schema, "LRW00000000000002"}
    ] do
  {:ok, tenant} =
    Tenant.new(%{
      id: schema,
      ...
      vehicles: [%{id: "9001", vin: vin, status: "active"}]
    })
  ...
  TenantContext.run(schema, fn ->
    ...
    for {seconds, fields} <- [
          {1, %{"Gear" => "ShiftStateD", "VehicleSpeed" => 30.0}},
          {61, %{"Odometer" => 10001.123456}},
          {121, %{"Gear" => "ShiftStateP", "Odometer" => 10002.123456, "VehicleSpeed" => 0.0}}
        ] do
...
IO.puts(
  "Shared Fleet projection: two tenants, identical IDs, independent drives and restart checkpoints verified"
)
